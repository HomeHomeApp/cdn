	...
	function getVersion(myObj)
	{
		console.log(JSON.stringify(myObj));
		ver = myObj.version;
		doit();
		document.getElementById("latest").innerHTML = "Latest &nbsp&nbspFirmware Version : " + ver
	}
	...
	function doit()
	{
		document.title = "HomeHome.App HTTP Server 1.0";
		var div = document.querySelector('#h');
		var make = '';
		make += "<button class=back  onclick=\"location.href='/" + getCookie("who") + "'\">&#8617;</button>";
		make += '<span id=aa>Apple HomeKit  :';
		if (setler[0] == 1)
		{
			make += ' Enabled'
		}
		...
		make += '<br><br>Free memory : ' + setler[4] + " bytes";
		make += '<br><br>Device Type: ' + setler[2];
		var t = parseInt(setler[5]);
		var d = Math.floor(t / 86400),
			h = ('0' + Math.floor(t / 3600) % 24).slice(-2),
			m = ('0' + Math.floor(t / 60) % 60).slice(-2),
			s = ('0' + t % 60).slice(-2);
		console.log((d > 0 ? d + 'd ' : '') + (h > 0 ? h + ':' : '') + (m > 0 ? m + ':' : '') + (t > 60 ? s : s + 's'));
		var sure = (d > 0 ? d + 'd ' : '') + (h > 0 ? h + 'hr ' : 'hr ') + (m > 0 ? m + 'min ' : 'min ') + (t > 60 ? s + 'sec' : s + 'ms');
		make += '<br><br>Uptime : ' + sure;
		make += '<br><br>Current Firmware Version : ' + setler[6];
		make += '<br><br><span id=latest>Latest &nbspFirmware Version : ';
		console.log("ver ok ? =" + ver);
		var webversion = ver;
		make += webversion;
		make += '</span><br><br></span>';
		if (webversion != setler[6] && webversion != "")
		{
			make += "<br><br><p id=a>Update Available<br><button class=rst onclick=\location.href='/ota" + getCookie("who") + "' \ type='button'>Update Device</button><br><br>";
			make += "If there is an important update you will see in Announcements.<br><br>If your device is working fine DO NOT update!<br><br>If you have internet connection you can do update!<br><br>If you do not know what you are doing skip this. Be warned your device may unusable!<br></p>"
		}
		make += "<br><br>";
		var myname = setler[3].split('|');
		if (myname[0].length > 0)
		{
			myname[0] = decodeURIComponent(myname[0]);
			make += "<hr><br><span style='margin-right:10%;'>1.Device Name</span><br><input type='text' id='n1' maxlength=16 value='" + myname[0] + "'/><button onclick=saven1()>Save</button><br><br>"
		}
		if (myname[1].length > 0)
		{
			myname[1] = decodeURIComponent(myname[1]);
			make += "<hr><br><span style='margin-right:10%;'>2.Device Name</span><br><input type='text' id='n2' maxlength=16 value='" + myname[1] + "'/><button onclick=saven2()>Save</button><br><br>"
		}
		if (myname[2].length > 0)
		{
			myname[3] = decodeURIComponent(myname[3]);
			make += "<hr><br><span style='margin-right:10%;'>3.Device Name</span><br><input type='text' id='n3' maxlength=16 value='" + myname[2] + "'/><button onclick=saven3()>Save</button><br><br>"
		}
		if (myname[3].length > 0)
		{
			myname[4] = decodeURIComponent(myname[4]);
			make += "<hr><br><span style='margin-right:10%;'>4.Device Name</span><br><input type='text' id='n4' maxlength=16 value='" + myname[3] + "'/><button onclick=saven4()>Save</button><br><br>"
		}
		make += "<hr><br><span style='margin-right:10%;'>Page Redirect</span><br><input type='number' id='redir' min=1 max=10 placeholder='Page redirect minute'/><button onclick=setredir()>OK</button><br>";
		make += "<span>Page will be redirected after 1-10 minutes.<br>Check quick help for info.[Default 1]</span><br><br><hr>"
		...
		runreverse();
		localenabled();
		console.log("setler: " + setler)
	}

	function resetme()
	{
		var r = confirm("All settings will be removed. Continue?");
		if (r == !0)
		{
			window.location = "/reset" + getCookie("who")
		}
	}

	function saven1()
	{
		var namer = document.getElementById("n1").value;
		real_request("/mode/n1/" + namer)
	}

	function saven2()
	{
		var namer = document.getElementById("n2").value;
		real_request("/mode/n2/" + namer)
	}

	function saven3()
	{
		var namer = document.getElementById("n3").value;
		real_request("/mode/n3/" + namer)
	}

	function saven4()
	{
		var namer = document.getElementById("n4").value;
		real_request("/mode/n4/" + namer)
	}

	function runreverse()
	{
		var kacrole = parseInt(setler[9]);
		if (kacrole > 0)
		{
			if (kacrole == 1)
			{
				...
				{
					document.getElementById("reverse1").checked = !0
				}
			}
			if (kacrole == 2)
			{
				...
				{
					document.getElementById("reverse2").checked = !0
				}
			}
			if (kacrole == 4)
			{
				...
				{
					document.getElementById("reverse3").checked = !0
				}
				if (setler[13] == "0")
				{
				...
				{
					document.getElementById("reverse4").checked = !0
				}
			}
		}
	}
	...
	function local()
	{
		var r = confirm("Restart required for local mode changes. Continue?");
		if (r == !0)
		{
			...
			{
				real_request("/mode/ghon")
			}
			setTimeout(function()
			{
				window.location = "/restart" + getCookie("who")
			}, 1000)
		}
	}
	...
	function skip()
	{
		...
		{
			document.getElementById("skipme").checked = !0
		}
	}
	...
	function localenabled()
	{
		...
		{
			document.getElementById("localmode").checked = !0
		}
	}
	...
	function turnme()
	{
		...
		{
			document.getElementById("turn").checked = !0
		}
	}
	...
	function turnback()
	{
		...
		{
			real_request("/turnbackoff")
		}
	}

	function real_request(uri)
	{
		uri = uri + getCookie("who");
		let xhr = new XMLHttpRequest();
		console.log("istenen " + uri);
		xhr.open('GET', uri);
		xhr.timeout = 1000;
		...
		xhr.onload = function()
		{
			...
			if (xhr.readyState === xhr.DONE)
			{
				if (xhr.status === 200)
				{
					var yaz = xhr.response;
					console.log("xhr.response " + yaz)
				}
			}
		};
	...
	var timeoutInMiliseconds = 10 * 60000;
	var timeoutId;

	function setredir()
	{
		setCookie("redir", document.getElementById("redir").value, "360");
		redir()
	}

	function redir()
	{
		var redir = parseInt(getCookie("redir"));
		if (redir > 0)
		{
		...
		{
			setCookie("redir", "1", "360");
			console.log("redir set")
		}
	}
	...
	(function()
	{
		...
		favlink.href = 'https://cdn.jsdelivr.net/gh/HomeHomeApp/cdn@latest/favicon.ico';
	...
	(function()
	{
		...
		link.type = 'text/css';
		link.rel = 'stylesheet';
		link.href = 'https://cdn.jsdelivr.net/gh/HomeHomeApp/cdn@latest/style.css' + nocache;
		document.getElementsByTagName('head')[0].appendChild(link)
	})();

	function anime()
	{
		var anime = getCookie("anime");
		anime = "no";
		if (anime == "yes")
		{}
		else if (anime == "no")
		{
			document.getElementById("iso").classList.remove("blur");
			document.getElementById("aa").style.backgroundColor = "transparent";
			document.body.style.backgroundImage = "linear-gradient(to right top, #d5d4e5, #cacbe5, #bdc2e5, #afbae6, #9fb2e6, #99b5e8, #92b7ea, #8cbaeb, #96c7ed, #a5d3ee, #b8def0, #cce9f2) !important;";
			document.body.style.border = "none"
		}
	}
